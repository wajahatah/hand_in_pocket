...
FUSION_MODEL_PATH = "rf_models/hybrid_3dcnn_kpnorm_pos_gen_model-1.pth"
        ...
        for result in results:
            ...
            for kp_tensor in keypoint_tensor:
                ...
                if len(sliding_window[position]) == WINDOW_SIZE:
                    ...
                    position_a = pos_list[0]
                    position_b = pos_list[1]
                    position_c = pos_list[2]
                    position_d = pos_list[3]
                    position_tensor = torch.tensor([[position_a, position_b, position_c, position_d]], dtype=torch.float32, device=DEVICE)
                    keypoint_input = torch.cat([kpts_tensor, position_tensor], dim=1)

                    # print(f"keypoint {keypoint_input}")
                    # print("====================================")
