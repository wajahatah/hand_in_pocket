...
if __name__ == "__main__":
    ...
    mlp_model = load_mlp_model("rf_models/mlp_temp_norm_pos_gen-c4.pt", device)
                ...
                for person_idx, kp_tensor in enumerate(keypoints_tensor):
                    ...
                    if len(sliding_window[position]) == WINDOW_SIZE:
                        ...
                        for t in range(WINDOW_SIZE):
                            ...
                            for i in range(10):
                                flat_feature[f"kp_{i}_x_t{t}"] = sliding_window[position][t].get(f"kp_{i}_x", -1.0)
                            for i in range(10):
                                flat_feature[f"kp_{i}_y_t{t}"] = sliding_window[position][t].get(f"kp_{i}_y", -1.0)
                        ...
                        for t in range(5):  # For t0 to t4
                            # ordered_columns.append(f"position_t{t}")
                            ordered_columns.extend([f"kp_{i}_x_t{t}" for i in range(10)])
                            ordered_columns.extend([f"kp_{i}_y_t{t}" for i in range(10)])
